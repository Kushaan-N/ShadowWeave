% ShadowWeave workshop paper -- compressed 4-page version.
...
not free (Depth-Anything-V2-Small: 49.9\,ms p50 / 50.7\,ms p95 in isolation, evaluation-era
GTX~1080~Ti, $480{\times}640$), so deployment runs depth asynchronously while the 7\,ms
completion path consumes the latest depth frame at 20\,Hz (audio synthesis excluded). A falling-object \emph{proxy} (cell-level detection 0.787 at 2.62\,s mean lead;
...
without instrumentation), so we make no quantitative real-world claim; the model performs amodal
