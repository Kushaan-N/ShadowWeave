...
by the obstacles it moves among (``shadow''); a world model of only the visible surface is blind
to half its workspace. We study how well a model can \textbf{complete} occupancy there from a
...
\emph{temporal forecasting} shows the gain is almost entirely completion --- the pure-forecasting
residual is small ($\le$+0.007 at 5\,s). Randomizing room geometry per episode leaves the gain
statistically unchanged on never-seen rooms, ruling out memorization of a fixed shell. The
completion path runs at 7\,ms (p95); end-to-end with an off-the-shelf monocular-depth network it
is ${\sim}18$\,Hz on 2017-era hardware. It feeds an eyes-free spatial-audio interface as a
demonstration; we release the methodology as a reusable protocol for honest occupancy-completion
claims.
\end{abstract}
...
observe --- the occupancy occluded by the obstacles it moves among. A world model of only the
visible surface is blind to half its workspace, and completing that hidden state is a prerequisite
for reasoning about what happens next; for any downstream use, that inference is trustworthy only
with an honest estimate of its own reliability: a confident hallucination of clear space behind a
wall is worse than a calibrated ``I don't know.'' Prior work addresses parts of this problem but
not their conjunction (\S\ref{sec:related}).
...
\item \textbf{A decomposition methodology} that separates completion from forecasting. It reveals that the
  ...
  at or near zero (fixed static +0.000; moving tier +0.007 [+0.002, +0.012] at 5\,s ---
...
--- occupied at \emph{no} horizon ($\approx$92\%), the false-positive test. (Dynamics faster than
the 1\,s grid --- e.g.\ debris settling within a second --- fall into STATIC, biasing the split
toward completion; the continuously-moving tier is the cleaner forecasting test.) This separates
...
mask at 5\,s, in-shadow ECE is 0.043--0.047 --- lower than observed space (0.058--0.086), Brier
0.022--0.036 --- though this edge is aided by shadow's emptiness. Calibration is no fixed-room
artifact: on randomized geometry, in-shadow ECE is 0.050--0.067 vs observed 0.056--0.077, shadow
better-calibrated at 5--10\,s (comparable at 1\,s).
...
redundant (no optical flow on the critical path); as independent retrainings that all land
