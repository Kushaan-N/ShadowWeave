...
by the obstacles it is moving among (the ``shadow''), and a world model of only the visible surface
is blind to half of its workspace. We ask how well a model can \textbf{complete} occupancy there
from a \textbf{single monocular-depth frame}, and whether it knows \emph{when it is guessing}.
...
0.043--0.047, at 5\,s) than in directly observed space (0.058--0.086)}. Framing this as world
modeling of the unobserved, a decomposition separating \emph{amodal completion} from
\emph{temporal forecasting} shows the gain is almost entirely completion, and the pure-forecasting
residual is small ($\le$+0.007 at 5\,s). Randomizing room geometry per episode leaves the gain
statistically unchanged on never-seen rooms, which rules out memorization of a fixed shell. The
completion path runs at 7\,ms (p95), and end-to-end with an off-the-shelf monocular-depth network
it is ${\sim}18$\,Hz on 2017-era hardware. It feeds an eyes-free spatial-audio interface as a
demonstration, and we release the methodology as a reusable protocol for honest
...
Any embodied agent acting in the physical world must reason about space it cannot directly
observe: the occupancy occluded by the obstacles it is moving among. A world model of only the
visible surface is blind to half of its workspace, and completing that hidden state is a
prerequisite for reasoning about what happens next. For any downstream use, that inference is
trustworthy only with an honest estimate of its own reliability, since a confident hallucination of
clear space behind a wall is worse than a calibrated ``I don't know.'' Prior work addresses parts
of this problem but not their conjunction (\S\ref{sec:related}).
\textbf{ShadowWeave}, a world model of occluded space, produces from a single monocular-depth
frame (i) an egocentric BEV occupancy grid, (ii) an explicit shadow mask marking cells the sensor
cannot see, and (iii) a calibrated per-cell occupancy probability inside that mask. Our
contributions are:
...
\item \textbf{A decomposition methodology} that separates completion from forecasting. It reveals that the
  ...
  at or near zero (fixed static +0.000; moving tier +0.007 [+0.002, +0.012] at 5\,s, which is
  significant but practically small, and decays with horizon as real forecasting should). We report
  this negative honestly, since most pooled occupancy numbers in the literature share exactly this
  confound.
...
\item \textbf{A real-time completion front-end} (7\,ms p95 completion path, well inside 20\,Hz;
  ${\sim}18$\,Hz end-to-end with off-the-shelf depth on 2017-era hardware), feeding an A* +
  nine-zone HRTF eyes-free interface as a systems demonstration (\S\ref{sec:experiments}).
\end{itemize}
...
no multi-frame memory, which makes the completion-vs-forecasting distinction
...
sub-masks. We therefore report \textbf{micro-averaged} shadow IoU: we pool integer intersection
and union counts across all frames and divide once (empty denominators are undefined, never 1.0),
and the defended quantity is the \textbf{gain} over a persistence baseline (the observed frame
...
is classified by its occupancy pattern across horizons. \textbf{STATIC} cells are occupied at
\emph{all} horizons (persistent hidden structure; $\approx$7\% of shadow), where recall measures
\textbf{amodal completion}; \textbf{DYNAMIC} cells are occupied at \emph{some but not all} horizons
($\le$1.5\%), where IoU measures genuine \textbf{forecasting} beyond persistence; and \textbf{NEVER}
cells are occupied at \emph{no} horizon ($\approx$92\%), the false-positive test. (Dynamics faster
than the 1\,s grid, for example debris settling within a second, fall into STATIC, biasing the
split toward completion; the continuously-moving tier is the cleaner forecasting test.) This
separates completion from clairvoyance, a distinction pooled shadow-IoU hides.
...
obstacles), \textbf{moving} (kinematic movers), and \textbf{debris} (objects that fall and settle).
It has 400 training / 80 validation episodes on disjoint seeds; per-tier analyses use 30-episode
(9{,}000-frame) single-tier sets, randomized-geometry validation uses 80 episodes, and
fixed-geometry results use a $6\times6$\,m room unless noted. Reproducibility is pinned: scene
generation is bit-exact against a golden hash, and a rollout re-run reproduces every accuracy
metric bit-identically (hardware-dependent latency excepted).

\textbf{Shadow-gain headline.} The model beats persistence at every horizon
(Fig.~\ref{fig:shadowgain}), with every bootstrap interval clear of zero, so the gain is
statistically significant, not sampling noise. In policy rollouts the
...
randomized-geometry set): to recall hidden structure (0.86--0.92) it must blanket 72--79\% of
genuinely empty hidden space with false positives, whereas the model achieves 0.61--0.72 IoU at
a 1.4--3.4\% false-positive rate. An observation-\emph{conditioned} heuristic (dilating observed
obstacles into shadow, scored at its best radius) peaks at 0.31--0.35, at or below
persistence itself. Completion is thus learned observation-conditional inference; with geometry
randomization (below), this rules out memorized layouts, marginals, and local extrapolation
alike.
...
residual ``nostatic'' gain is at or near zero: fixed static +0.000, moving +0.007 (CI
...
entirely) and completion recall is if anything higher on never-seen rooms, so completion is
per-scene inference, not a memorized shell.

\textbf{Calibration in shadow.} Shadow is ${\sim}$92\% empty, so a raw in-shadow ECE is easy to
make small; we therefore lead with the harder number. Excluding persistent structure entirely,
in-shadow ECE at 5\,s remains a low 0.055--0.056 on the tiers with dynamic content (the static
tier's structure-excluded region has no positive labels, hence no curve), so calibration is
carried by the genuinely uncertain cells, not the easy always-occupied ones. On the full shadow
mask at 5\,s, in-shadow ECE is 0.043--0.047, lower than observed space (0.058--0.086), with Brier
0.022--0.036, though this edge is aided by shadow's emptiness. Calibration is no fixed-room
artifact: on randomized geometry, in-shadow ECE is 0.050--0.067 vs observed 0.056--0.077, and
shadow is better-calibrated at 5--10\,s (comparable at 1\,s).
...
redundant (no optical flow on the critical path); and as independent retrainings that all land
within the full model's CIs, they also bound training-seed variance.

\textbf{System.} The perception-to-planning path runs at 6.47\,ms p50 / 6.99\,ms p95 (batch 1,
...
(audio synthesis excluded). An occupancy-arrival \emph{diagnostic} (not falling-object anticipation; cell-level detection 0.787 at 2.62\,s mean lead;
...
$\ge$3\,s target; we report it as a diagnostic, not a strength. Navigation reaches a per-episode
collision rate of 0.367 (vs 0.60 untrained; straightness 0.773), missing the $\le$0.10 target, so
it is an honest demonstration and no more.
...
single image, but targets outdoor driving voxels evaluated purely by mIoU/IoU, without calibrated
uncertainty in the occluded region. Occupancy world models such as OccWorld \citep{zheng2024occworld}, together with occupancy-forecasting
...
evaluation is in simulation on clean simulator depth (robustness to estimated-depth noise is
untested), so we make no quantitative real-world claim, though the front-end
already accepts real monocular depth (Depth-Anything-V2), making sim-to-real transfer the
natural next step; baselines are self-constructed (prior methods target different input
regimes); the model performs amodal
...
the occupancy-arrival lead is bounded by the horizon grid.
