...
completion model is the natural next step — noting that evaluation currently consumes the
simulator's clean depth, so robustness of the completion to estimated-depth noise is untested.
We compare against strong self-constructed baselines rather than prior methods, which target
different input regimes (e.g., ProxMaP's top-down RGB-D view); a head-to-head on a shared
